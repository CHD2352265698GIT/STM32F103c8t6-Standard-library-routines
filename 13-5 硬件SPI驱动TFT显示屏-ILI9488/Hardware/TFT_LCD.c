...
{
	RCC_APB2PeriphClockCmd(ST7735_RST_GPIO_RCC, ENABLE);
	RCC_APB2PeriphClockCmd(ST7735_DC_GPIO_RCC, ENABLE);
	RCC_APB2PeriphClockCmd(ST7735_CS_GPIO_RCC, ENABLE);
	RCC_APB2PeriphClockCmd(ST7735_BL_GPIO_RCC, ENABLE);
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(ST7735_BL_GPIO_Port, &GPIO_InitStructure);

	GPIO_SetBits(ST7735_BL_GPIO_Port, ST7735_BL_Pin);
 ...
 void ST7735_WriteCommand(uint8_t cmd)
{
  ...
  GPIO_ResetBits(ST7735_CS_GPIO_Port, ST7735_CS_Pin);
  MySPI_SwapByte(cmd);
  GPIO_SetBits(ST7735_CS_GPIO_Port, ST7735_CS_Pin);
}
...
void ST7735_WriteData(uint8_t *data, unsigned int data_size)
{
  GPIO_SetBits(ST7735_DC_GPIO_Port, ST7735_DC_Pin);
  GPIO_ResetBits(ST7735_CS_GPIO_Port, ST7735_CS_Pin);
  MySPI_SendStr(data, data_size);
  GPIO_SetBits(ST7735_CS_GPIO_Port, ST7735_CS_Pin);
}
